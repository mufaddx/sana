...
import { type CareerField, type Option, type Stream, buildQuestions, careerFields, streamDetails } from '@/data/assessment';
...
const defaultPublicSettings = {
  ...
  hero_description: 'Explore your interests, understand your strengths and discover skill fields that may be a strong fit for you.',
  contact_reply_window: 'Tell us what is on your mind. We will keep the conversation clear and student-friendly.',
};
...
function Header() {
  ...
  const links = [['How It Works', '#how-it-works'], ['Skill Fields', '#career-fields'], ['About', '#about'], ['FAQ', '#faq'], ['Contact', '#contact']];
...
function Home() {
    ...
    fetch('/api/public/settings')
      ...
      .then((payload) => {
        if (payload?.settings && typeof payload.settings === 'object') {
          setPublicSettings((current) => ({ ...current, ...payload.settings }));
        }
      })
      .catch(() => undefined);
  }, []);
  const handleContact = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(typeof body?.message === 'string' ? body.message : 'Your message could not be sent.');
      setContactSent(true);
    } catch (error) {
  ...
  const faqs = [
    ['What is Real World Link?', 'Real World Link is a student-friendly skill-discovery space from In Shorts by Aafiya & Sana. It helps you connect what interests you with possible skill directions.'],
    ['Who can take the assessment?', 'Students exploring their next academic or skill direction can take it. You do not need to have a final answer before you begin.'],
    ['How long does the assessment take?', 'Most students can move through the 20-question experience in around 5–8 minutes.'],
    ['How does the assessment work?', 'You choose a broad stream, answer questions about the kind of work and ideas that interest you, then receive a best-fit skill field to explore.'],
    ['How is the best-fit field selected?', 'Your responses are connected to the skill examples within your selected stream. The result reflects patterns in your answers, not a fixed prediction.'],
    ['Is the result guaranteed?', 'No. It is a starting point for skill exploration, not a guarantee, diagnosis or final decision about your future.'],
  ...
  return <div className="site-shell" id="top"><Header />
    <main>
       <section className="hero"><div className="container-rwl hero-grid"><div className="reveal"><h1>{publicSettings.hero_title}</h1><p className="hero-copy">{publicSettings.hero_description}</p><div className="hero-actions"><Link href="/assessment" className="button-primary" data-testid="link-hero-assessment">Start Your Assessment <ArrowRight size={16} /></Link><a href="#career-fields" className="button-secondary" data-testid="link-hero-fields">Explore Skill Fields</a></div><div className="hero-note"><ShieldCheck size={15} /> A thoughtful starting point, never a promise about your future.</div></div><div className="hero-visual reveal"><div className="visual-main photo-main"><img src={studentPhoto} alt="Student exploring her future path" className="hero-student-photo" /></div><div className="visual-card top"><span className="card-label">Your next question</span><strong>What makes you curious?</strong></div><div className="visual-card bottom"><span className="card-label">Skill discovery</span><strong>Based on your interests</strong><div className="mini-path"><i /><i /><i /></div></div></div></div></section>
      <section className="value-strip"><div className="container-rwl value-grid">{[[GraduationCap, 'Student-focused'], [Search, 'Skill exploration'], [ClipboardCheck, 'Simple assessment'], [Radar, 'Future-oriented']].map(([Icon, label], index) => <div className="value-item" key={label as string} data-testid={`value-${index}`}><Icon size={17} /><span>{label as string}</span></div>)}</div></section>
      <section className="section" id="about"><div className="container-rwl split-grid"><div className="about-visual"><div className="about-panel"><div className="about-panel-line" /><div className="about-panel-line short" /><div className="about-panel-pill"><Target size={14} /> Your direction starts here</div></div></div><div className="about-copy"><div className="eyebrow section-kicker">A clearer beginning</div><h2 className="section-title">What is Real World Link?</h2><blockquote>Choosing a skill starts with understanding yourself.</blockquote><p className="section-copy">We connect interests, strengths and subjects with real-world possibilities — so your next step feels more considered, not more pressured.</p><div className="about-points">{['Interests', 'Strengths', 'Subjects', 'Skill fields'].map((point) => <span className="point-tag" key={point}>{point}</span>)}</div></div></div></section>
      <section className="section section-tint" id="how-it-works"><div className="container-rwl"><div className="section-head"><div><div className="eyebrow section-kicker">A simple four-step journey</div><h2 className="section-title">Make room for<br />a better question.</h2></div><p className="section-copy">There is no pressure to have everything figured out. Just start with what catches your attention.</p></div><div className="steps-grid">{[['01', Lightbulb, 'Discover yourself', 'Notice the subjects, ideas and problems that keep your curiosity alive.'], ['02', ClipboardCheck, 'Take the assessment', 'Answer simple skill-discovery questions, one at a time.'], ['03', Target, 'Explore your fit', 'See the skill fields that may align with your responses.'], ['04', Users, 'Connect with our team', 'Use your result as a conversation starter for what comes next.']].map(([number, Icon, title, copy]) => <div className="step" key={number as string}><div className="step-number">{number as string}</div><h3>{title as string}</h3><p>{copy as string}</p></div>)}</div></div></section>
      <section className="section" id="career-fields"><div className="container-rwl"><div className="section-head"><div><div className="eyebrow section-kicker">Four places to begin</div><h2 className="section-title">Possible paths,<br />not fixed labels.</h2></div><p className="section-copy">Explore the fields, then let your questions lead the way. Every field holds more than one kind of future.</p></div><div className="career-grid">{careerFields.map((field) => <button className={`skill-card skill-${field.id}`} onClick={() => setSelectedField(field)} key={field.id} data-testid={`button-career-${field.id}`}><span className="career-icon"><field.icon size={22} /></span><span className="career-arrow"><ArrowUp size={17} style={{ transform: 'rotate(45deg)' }} /></span><h3>{field.title}</h3><p>{field.description}</p><div className="tag-row">{field.careers.map((career) => <span className="tag" key={career}>{career}</span>)}</div></button>)}</div></div></section>
      <section className="section section-lilac"><div className="container-rwl why-grid"><div><div className="eyebrow section-kicker">A thoughtful first step</div><h2 className="section-title">Why start<br />with yourself?</h2><p className="section-copy">Because a useful direction is easier to find when it begins with what feels like you — not what sounds impressive on paper.</p><div className="journey">{['Curiosity', 'Assessment', 'Discovery', 'Direction', 'Next step'].map((item, index) => <div className="journey-item" key={item}><span className="journey-dot">{index + 1}</span><strong>{item}</strong></div>)}</div></div><div className="why-list">{[[Brain, 'Understand your interests', 'Put language around what draws your attention.'], [Globe2, 'Explore skill possibilities', 'Meet fields you may not have considered yet.'], [Scale, 'Make better-informed choices', 'Move from outside pressure to useful context.'], [Zap, 'Discover new directions', 'Let a surprising answer become a new question.']].map(([Icon, title, copy]) => <div className="why-card" key={title as string}><Icon size={21} /><h3>{title as string}</h3><p>{copy as string}</p></div>)}</div></div></section>
      <section className="cta-band"><div className="container-rwl cta-inner"><div><div className="eyebrow">Your next step can be small</div><h2 className="section-title">Not sure where your interests can take you?</h2><p>Take a few minutes to explore which skill field may align with your interests.</p></div><Link href="/assessment" className="button-primary" data-testid="link-cta-assessment">Start My Assessment <ArrowRight size={16} /></Link></div></section>
      <section className="section" id="about-team"><div className="container-rwl"><div className="eyebrow section-kicker">In Shorts by Aafiya &amp; Sana</div><h2 className="section-title">Meet the team behind<br />Real World Link.</h2><div className="team-grid"><div className="team-card"><div className="team-avatar">A</div><div><h3>Aafiya</h3><div className="team-role">Lead Mentor</div><p>Helping students turn curiosity into a clearer conversation about what could come next.</p></div></div><div className="team-card"><div className="team-avatar">S</div><div><h3>Sana</h3><div className="team-role">Lead Mentor</div><p>Creating thoughtful spaces for students to explore strengths, questions and possibilities.</p></div></div></div></div></section>
       <section className="section section-tint faq-contact-section" id="faq"><div className="container-rwl faq-contact-grid"><div className="faq-column"><div className="faq-list">{faqs.map(([question, answer], index) => <div className="faq-item" key={question}><button className="faq-button" aria-expanded={faqOpen === index} onClick={() => setFaqOpen(faqOpen === index ? null : index)} data-testid={`button-faq-${index}`}><span>{question}</span><ChevronDown size={17} /></button><div className={`faq-answer ${faqOpen === index ? 'open' : ''}`}><p>{answer}</p></div></div>)}</div></div><div className="contact-card faq-contact-card" id="contact"><div className="eyebrow section-kicker">Get in touch</div>{contactSent ? <div className="success-state"><div className="success-icon"><Check size={29} /></div><h3>Thank you. Your message has been received.</h3><p>We appreciate you reaching out to Real World Link.</p><button className="button-secondary contact-submit" onClick={() => { setContactSent(false); setContactError(''); }} data-testid="button-send-another">Send another message</button></div> : <><h2 className="contact-form-title">Have a question?</h2><p className="contact-form-copy">{publicSettings.contact_reply_window}</p><form onSubmit={handleContact}><div className="form-grid"><div className="field"><label htmlFor="contact-name">Name</label><input id="contact-name" name="name" required placeholder="Your name" data-testid="input-contact-name" /></div><div className="field"><label htmlFor="contact-email">Email</label><input id="contact-email" name="email" type="email" required placeholder="you@example.com" data-testid="input-contact-email" /></div><div className="field"><label htmlFor="contact-subject">Subject</label><input id="contact-subject" name="subject" required placeholder="What would you like to ask?" data-testid="input-contact-subject" /></div><div className="field"><label htmlFor="contact-message">Message</label><textarea id="contact-message" name="message" required placeholder="Write a little context..." data-testid="input-contact-message" /></div></div>{contactError && <div className="error-text" role="alert" data-testid="text-contact-error">{contactError}</div>}<button type="submit" className="button-primary contact-submit" disabled={contactSubmitting} data-testid="button-send-message"><Send size={15} /> {contactSubmitting ? 'Sending…' : 'Send Message'}</button></form></>}</div></div></section>
    </main>
...
function CareerModal({ field, onClose }: { field: CareerField; onClose: () => void }) {
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><div className="career-modal" role="dialog" aria-modal="true" aria-labelledby="career-modal-title"><button className="modal-close" onClick={onClose} aria-label="Close skill field details" data-testid="button-close-career-modal"><X size={18} /></button><div className="modal-icon"><field.icon size={25} /></div><div className="eyebrow section-kicker">{field.title}</div><h2 id="career-modal-title">A field for your kind of curiosity.</h2><p>{field.description} Explore the examples below as possible directions, not as labels you have to grow into.</p><div className="modal-label">Skill examples</div><ul className="modal-list">{field.careers.map((career) => <li key={career}>{career}</li>)}</ul><div className="modal-label">Potential interests</div><div className="tag-row">{field.interests.map((interest) => <span className="point-tag" key={interest}>{interest}</span>)}</div><Link href="/assessment" className="button-primary" style={{ marginTop: 25 }} onClick={onClose} data-testid="link-modal-assessment">Take the Assessment <ArrowRight size={16} /></Link></div></div>;
}
...
function AssessmentPage() {
  ...
  const [student, setStudent] = useState<StudentInfo>({ name: '', email: '', grade: '', city: '', school: '', phone: '', address: '', state: '', pincode: '', consent: false });
  ...
  const continueInfo = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!student.phone || !student.address || !student.state || !student.pincode) return setInfoError('Please complete the delivery details so we can send your Linking Box.');
    if (student.phone.replace(/[^\d]/g, '').length < 10) return setInfoError('Enter a valid phone number.');
    if (!/^\d{4,10}$/.test(student.pincode)) return setInfoError('Enter a valid PIN code.');
    setInfoError('');
    setStage('questions');
  };
  const finish = async () => {
    ...
    const nextResult = { primary: ranked[0]?.[0] || field.careers[0], secondary: ranked[1]?.[0] || field.careers[1] };
    ...
    try {
      const response = await fetch('/api/public/assessment', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
        body: JSON.stringify({ ...student, stream, result: nextResult.primary, answers }),
      });
      const body = await response.json().catch(() => null);
      if (!response.ok) throw new Error(typeof body?.message === 'string' ? body.message : 'Your assessment could not be submitted.');
      if (typeof body?.trackingCode === 'string') setTrackingCode(body.trackingCode);
      setResult(nextResult);
      setStage('analysis');
      window.setTimeout(() => setStage('result'), 1900);
    } catch (error) {
      setSubmitError(error instanceof Error ? error.message : 'Your assessment could not be submitted.');
    } finally {
  ...
  return <div className="assessment-shell"><AssessmentHeader /><main className="assessment-main">
    ...
    {stage === 'questions' && stream && <div className="assessment-page reveal" key={questionIndex}><div className="question-kicker">Career discovery assessment · {stream}</div><div className="progress-wrap"><div className="progress-label"><span>Question {questionIndex + 1} of 20</span><span>{Math.round(((questionIndex + 1) / 20) * 100)}% complete</span></div><div className="progress-track"><div className="progress-fill" style={{ width: `${((questionIndex + 1) / 20) * 100}%` }} /></div></div><div className="question-title">{questionIndex === 0 ? 'Which broad direction would you like to explore?' : questions[questionIndex - 1].prompt}</div><div className="answer-grid">{(questionIndex === 0 ? streamDetails[stream].field.careers.map((career) => ({ career, text: career })) : questions[questionIndex - 1].options).map((option, index) => <button className={`choice-card answer-card ${answers[questionIndex]?.career === option.career ? 'selected' : ''}`} onClick={() => chooseAnswer(option)} key={`${option.career}-${index}`} data-testid={`button-answer-${questionIndex}-${index}`}><span className="choice-icon">{index === 0 ? <Target size={17} /> : index === 1 ? <Lightbulb size={17} /> : index === 2 ? <Search size={17} /> : <Sparkles size={17} />}</span><strong>{option.text}</strong>{answers[questionIndex]?.career === option.career && <Check className="answer-check" size={18} />}</button>)}</div><div className="assessment-actions"><button className="button-secondary" disabled={questionIndex === 0} onClick={() => { setDirection('previous'); setQuestionIndex((index) => Math.max(0, index - 1)); }} data-testid="button-previous-question"><ArrowLeft size={16} /> Previous</button><button className="button-primary" disabled={!answers[questionIndex]} onClick={() => { if (questionIndex === 19) setStage('review'); else { setDirection('next'); setQuestionIndex((index) => index + 1); } }} data-testid="button-next-question">{questionIndex === 19 ? 'Review & Submit' : 'Next'} <ArrowRight size={16} /></button></div><span className="sr-only">{direction}</span></div>}
     {stage === 'review' && stream && <div className="assessment-page reveal"><div className="eyebrow">Step 3 of 3 · Ready when you are</div><h1>Review &amp; submit.</h1><p>Your answers are saved. Take a breath, check the details below and submit when it feels right.</p><div className="review-card"><div className="review-row"><span>Status</span><strong><CheckCircle2 size={15} style={{ verticalAlign: 'middle', color: 'hsl(169 48% 35%)' }} /> Assessment complete</strong></div><div className="review-row"><span>Questions answered</span><strong>20 / 20</strong></div><div className="review-row"><span>Student name</span><strong>{student.name}</strong></div><div className="review-row"><span>Email</span><strong>{student.email}</strong></div><div className="review-row"><span>Selected stream</span><strong>{stream}</strong></div></div>{submitError && <div className="error-text" role="alert" data-testid="text-assessment-submit-error">{submitError}</div>}<div className="assessment-actions"><button className="button-secondary" onClick={() => { setQuestionIndex(0); setStage('questions'); }} data-testid="button-review-answers"><ArrowLeft size={16} /> Review Answers</button><button className="button-primary" disabled={isSubmitting} onClick={() => void finish()} data-testid="button-submit-assessment">{isSubmitting ? 'Submitting…' : 'Submit Assessment'} {!isSubmitting && <ArrowRight size={16} />}</button></div></div>}
    {stage === 'analysis' && <div className="assessment-intro reveal"><div className="eyebrow">One moment</div><h1>Reading your responses.</h1><p>We are grouping your interests into possible skill directions.</p><div className="analysis-list"><div className="analysis-item"><CheckCircle2 size={18} /> Analyzing your responses</div><div className="analysis-item"><CheckCircle2 size={18} /> Understanding your interests</div><div className="analysis-item"><CheckCircle2 size={18} /> Finding your best-fit skill field</div></div></div>}
...
function Router() {
  ...
  return <ErrorBoundary resetKey={location}><Switch><Route path="/" component={Home} /><Route path="/assessment" component={AssessmentPage} /><Route path="/track" component={TrackPage} /><Route path="/admin" component={AdminPage} /><Route path="/privacy" component={() => <LegalPage kind="privacy" />} /><Route path="/terms" component={() => <LegalPage kind="terms" />} /><Route component={NotFound} /></Switch></ErrorBoundary>;
}
